...
RSpec.describe "Api::V1::CookingRecipes", type: :request do
  ...
  describe "GET /api/v1/cooking-recipes/:id" do
    before do
      cooking_recipe
      get "/api/v1/cooking-recipes/#{cooking_recipe.id}"
    end
  ...
  describe "PATCH /api/v1/cooking-recipes/:id" do
    let(:new_title) { Faker::Food.dish }
    let(:valid_attributes) { { title: new_title } }
    let(:invalid_attributes) { { title: nil } }

    it "responds with status code 401" do
      patch "/api/v1/cooking-recipes/#{cooking_recipe.id}", params: { cooking_recipe: valid_attributes }
      expect(response).to have_http_status(:unauthorized)
    end

    context "when the user is logged in" do
      it "responds with status code 401" do
        patch "/api/v1/cooking-recipes/#{cooking_recipe.id}", params: { cooking_recipe: valid_attributes }, headers: auth_headers
        expect(response).to have_http_status(:unauthorized)
      end
      ...
      context "when the user is an admin" do
        ...
        context "with invalid parameters" do
          it "responds with status code 422" do
            patch "/api/v1/cooking-recipes/#{cooking_recipe.id}", params: { cooking_recipe: invalid_attributes }, headers: auth_headers
            expect(response).to have_http_status(:unprocessable_entity)
          end
        end
      end
  ...
  describe "DELETE /api/v1/cooking-recipes/:id" do
    before { cooking_recipe }

    it "responds with status code 401" do
      delete "/api/v1/cooking-recipes/#{cooking_recipe.id}"
      expect(response).to have_http_status(:unauthorized)
    end

    context "when the user is logged in" do
      it "responds with status code 401" do
        delete "/api/v1/cooking-recipes/#{cooking_recipe.id}", headers: auth_headers
        expect(response).to have_http_status(:unauthorized)
      end
