...
RSpec.describe UpdateCiqualDataJob, type: :job do
  describe '#perform' do
    ...
    it 'saves ciqual food groups in the database' do
      expect do
        UpdateCiqualDataJob.perform_now
      end.to change(FoodGroup, :count).from(0).to(85)
    end

    it 'saves ciqual food groups code and label' do
      expect do
        UpdateCiqualDataJob.perform_now
      end.to change { FoodGroup.first&.as_json }.from(nil).to(
        match({
          "code" => "010301",
          "label" => "plats de viande sans garniture",
